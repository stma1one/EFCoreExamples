...
public partial class HighScoresContext : DbContext
{
    ...
    public virtual DbSet<PlayerHighScore> PlayerHighScores { get; set; }
...
#warning To protect potentially sensitive information in your connection string, you should move it out of source code. You can avoid scaffolding the connection string by using the Name= syntax to read it from configuration - see https://go.microsoft.com/fwlink/?linkid=2131148. For more guidance on storing connection strings, see http://go.microsoft.com/fwlink/?LinkId=723263.
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<PlayerHighScore>(entity =>
        {
            entity.HasKey(e => new { e.GameId, e.PlayerId }).HasName("PK_HighScore");

            entity.HasOne(d => d.Game).WithMany(p => p.PlayerHighScores)
                .OnDelete(DeleteBehavior.ClientSetNull)
                .HasConstraintName("FK_HighScore_Game");

            entity.HasOne(d => d.Player).WithMany(p => p.PlayerHighScores)
                .OnDelete(DeleteBehavior.ClientSetNull)
